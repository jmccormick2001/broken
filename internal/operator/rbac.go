...
func (r PipelineReconciler) processRoles(pipeline v1alpha1.Pipeline) error {
	...
	if err != nil {
		log.Error(err, "unable to list child roles")
		return err
	}
	...
	if needChurroRole {
		...
		*/
		...
		policyRule.Verbs = []string{"create", "get", "list"}
		policyRule.APIGroups = []string{""}
		policyRule.Resources = []string{"pods", "services", "secrets"}
		churroRole.Rules = append(churroRole.Rules, policyRule)

		policyRule = rbacv1.PolicyRule{}
		policyRule.Verbs = []string{"get", "list"}
	...
	for _, role := range rolesToCreate {
		...
		if err := r.Create(r.Ctx, &role); err != nil {
			log.Error(err, "unable to create rbacObject for pipeline", "rbac", role)
			return err
		}
		fmt.Println("created rbac for pipeline ")
	}
...
func (r PipelineReconciler) processServiceAccounts(pipeline v1alpha1.Pipeline) error {
	...
	for _, acct := range accountsToCreate {
		...
		if err := r.Create(r.Ctx, &acct); err != nil {
			log.Error(err, "unable to create service account for pipeline", "rbac", acct)
			return err
		}
		fmt.Println("created service account for pipeline ")
	}
...
func (r PipelineReconciler) processRoleBindings(pipeline v1alpha1.Pipeline) error {
	...
	for _, roleBinding := range roleBindingsToCreate {
		...
		if err := r.Create(r.Ctx, &roleBinding); err != nil {
			log.Error(err, "unable to create roleBinding for pipeline", "rbac", roleBinding)
			return err
		}
		fmt.Println("created rolebinding for pipeline ")
	}
