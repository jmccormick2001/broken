...
const (
	...
	loaderService          = "churro-loader"
	watchService           = "churro-watch"
...
func (r PipelineReconciler) processServices(pipeline v1alpha1.Pipeline) error {
	...
	needCtlService := true
	// make sure we have a service/churro-loader
	needLoaderService := true
	// make sure we have a service/churro-watch
	needWatchService := true
	...
	for i := 0; i < len(childServices.Items); i++ {
		...
		case loaderService:
			needLoaderService = false
		case watchService:
			needWatchService = false
	...
	if needWatchService {
		service := v1.Service{}
		service.ObjectMeta.Labels = map[string]string{"app": "churro", "pipeline": pipeline.ObjectMeta.Name, "service": "churro-watch"}
		service.Name = watchService
		service.Namespace = pipeline.ObjectMeta.Namespace
		...
		service.Spec.Ports = []v1.ServicePort{sp}
		service.Spec.Selector = map[string]string{"app": "churro", "pipeline": pipeline.ObjectMeta.Name, "service": "churro-watch"}
		/**
		  apiVersion: v1
		  kind: Service
		  metadata:
	...
	if needLoaderService {
		service := v1.Service{}
		service.ObjectMeta.Labels = map[string]string{"app": "churro", "pipeline": pipeline.ObjectMeta.Name, "service": "churro-loader"}
		...
		service.Spec.Ports = []v1.ServicePort{sp}
		service.Spec.Selector = map[string]string{"app": "churro", "pipeline": pipeline.ObjectMeta.Name, "service": "churro-loader"}
		/**
		  apiVersion: v1
		  kind: Service
		  metadata:
	...
	for _, svc := range servicesToCreate {
		...
		if err := r.Create(r.Ctx, &svc); err != nil {
			log.Error(err, "unable to create Service for pipeline", "service", svc)
			return err
		}
