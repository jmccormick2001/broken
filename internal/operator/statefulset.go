...
func (r PipelineReconciler) processStatefulSet(pipeline v1alpha1.Pipeline) error {
	...
	if needStatefulSet {
		...
		if err != nil {
			log.Error(err, "unable to unmarshal StatefulSet template")
			return err
		}
		...
		if err := r.Create(r.Ctx, &statefulset); err != nil {
			log.Error(err, "unable to create statefulset for pipeline", "statefulset", statefulset)
			return err
		}
...
func (r PipelineReconciler) initStatefulSet(req ctrl.Request, pipeline v1alpha1.Pipeline) error {
	...
	log.Info("initStatefulSet called")
	...
	if err != nil {
		log.Error(err, "unable to list child pods for StatefulSets")
		return err
	}
	...
	*/
	...
	if err != nil {
		log.Error(err, "unable to connect to kube")
		return err
	}
	podCheck := wait.ConditionFunc(func() (done bool, err error) {
		...
		if tmp.Status.Phase == corev1.PodRunning {
			fmt.Println("cockroachdb-0 pod is found to be running!")
			return true, nil
		}
		return false, nil
	})
	...
	if err != nil {
		log.Errorf("timeout waiting for %s to be running...", podName)
		podRunning = false
	} else {
		podRunning = true
	}

	fmt.Println("jeff:  cockroachdb-0 pod is deemed running")
	...
	if podRunning {
		...
		if err != nil {
			log.Error(err, "unable to get secret for StatefulSets")
		} else {
			secretFound = true
		}
		...
		if !secretFound {
			...
			if err := r.Create(r.Ctx, &secret); err != nil {
				log.Error(err, "unable to create init secret for statefulset", "configmap", secret)
				return err
			}
			fmt.Println("created init secret for statefulset")
			...
			printPods(ns, clientset)
			a, b, err := ExecToPodThroughAPI(config, clientset, command, containerName, podName, ns, nil)
			fmt.Printf("stdout %s stderr %s\n", a, b)
			if err != nil {
				log.Error(err, "error execing into cockroach for init")
				return err
			}
	...
	podReadyCheck := wait.ConditionFunc(func() (done bool, err error) {
		...
		for _, v := range tmp.Status.ContainerStatuses {
			if v.Ready {
				fmt.Println("cockroachdb-0 pod is found to be ready!")
				return true, nil
			}
		}
		return false, nil
	...
	if err != nil {
		log.Errorf("timeout waiting for %s to be ready...", podName)
		return err
	}
...
func ExecToPodThroughAPI(config *rest.Config, clientset *kubernetes.Clientset, command []string, containerName, podName, namespace string, stdin io.Reader) (string, string, error) {
	...
	log.Infof("Request object: %v", req)
	log.Infof("Request URL object: %v", req.URL())
...
func printPods(ns string, clientset *kubernetes.Clientset) {
	pods, err := clientset.CoreV1().Pods(ns).List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		log.Error(err, "error gettings pods in printPods")
	}
